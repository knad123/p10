...
def generate_files(conf, network_name, topology_data, simulation_directory, pkl_dir):
    ...
    mpls_network.create_RSVP_FN_protection()
...
def main(confs):
    ...
    if conf['algorithm'] in ["essence_split", "essence_learn_paths_learn_weights"]:
        ...
        conf['algorithm_and_parameters'] = conf['algorithm'] + "_p" + str(conf['population']).replace(".","_") + "_c" + str(conf['crossover']).replace(".","_") + "_m" + str(conf['mutation']).replace(".","_") + (f"_ui{conf['update_interval']}" if conf['update_interval'] is not None else "") + (f"_kp{str(conf['keep_percentage']).replace('.','_')}") + (f'_bf' if conf['big_flows'] else "") + f"_gen{conf['generations']}"
    else:
