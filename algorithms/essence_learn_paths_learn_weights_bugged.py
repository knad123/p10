...
def essence_learn_paths_learn_weights_bugged(network: MPLS_Network, essence_state: EssenceState, conf, start_time):
...
def genetic_algorithm(network, loads, capacities, essence_state, conf, start_time, generations=1000, population_size=200,
                      crossover_rate=0.9,
    ...
    if not essence_state.current_population:
        ...
        new_population = create_population(network, loads, int(population_size * 0.5), conf, essence_state, weight_range)
    ...
    essence_state.current_population = population[:int(len(population) * 0.5)]
